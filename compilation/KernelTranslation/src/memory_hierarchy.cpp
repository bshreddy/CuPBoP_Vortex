        ...
        if (AS == 3) {
          ...
          if (auto array_type = dyn_cast<ArrayType>(element_type)) {
            if (share_memory->hasExternalLinkage() &&
              ...
              llvm::GlobalVariable *global_memory = new llvm::GlobalVariable(
                  *M, array_type, false, llvm::GlobalValue::ExternalLinkage,
                  NULL, new_name, NULL, llvm::GlobalValue::LocalExecTLSModel,
                  1);
...
//  - Priority order:
//    (1) Compile-time constant initializer for grid_size_{x,y} (if available,
//    use it) (2) Hardcoded value matched by benchmark name (current
//    path/moduleID/source file) (3) Environment variable VORTEX_NUM_CTA (if
//    set, overrides everything) (4) Final fallback: 256
static uint64_t decideNumCTAsByBench(const llvm::Module &M) {
  ...
  if (contains("stencil"))
    return 30ull; // change to 180 if needed

  // transpose [grid=(64,64,1)]
  if (contains("transpose"))
    return 64ull * 64ull;
...
// (Originally assumed thread collapsing, but extended to work under HW thread
// mapping) Replace shared memory (AS=3) with global (AS=1). Create CTA slices
// of size grid_size_x * grid_size_y, and rewrite all IR uses so that
// block_index_x, block_index_y point to different slices per block.
//
...
//
// Note: If grid_size_* are not compile-time constants, the global array size
// cannot be determined → fallback value is used
...
void mem_share2global_sche_2(llvm::Module *M) {
  ...
  // 0) grid_size_x * grid_size_y → compute as compile-time constants if
  // possible
  ...
  if (!NumCTAConst) {
    std::cout << "[mem_share2global_sche_2] grid_size_{x,y} are not "
                 "compile-time constants. "
                 "Declaring global type size as "
              << NumCTAForType << " and computing indexing at runtime.\n";
  } else {
    std::cout
        << "[mem_share2global_sche_2] NumCTA = grid_size_x * grid_size_y = "
        << NumCTAForType << "\n";
  }
  ...
  auto createNoTLSGlobal = [&](Type *Ty, StringRef Name) -> GlobalVariable * {
    ...
    auto *GV = new GlobalVariable(
        *M, Ty, /*isConst=*/false, GlobalValue::ExternalLinkage, Init, Name,
  ...
  for (auto *SM : SharedGVs) {
    Type *ElemTy = SM->getValueType(); // LLVM18+: type of the GV’s "value"
    ...
    if (auto *ArrTy = dyn_cast<ArrayType>(ElemTy)) {
      ...
      if (SM->hasExternalLinkage() && ArrTy->getNumElements() == 0) {
        ...
        auto *DynTy =
            ArrayType::get(ArrayType::get(I8, MaxDynBytes), NumCTAForType);
        NewGV = createNoTLSGlobal(DynTy, "wrapper_global_dynbuf");
      } else {
        auto *SlicedTy = ArrayType::get(ArrTy, NumCTAForType);
        NewGV = createNoTLSGlobal(SlicedTy,
                                  "wrapper_global_" + SM->getName().str());
      }
    } else if (ElemTy->isIntegerTy() || ElemTy->isFloatTy() ||
               isa<PointerType>(ElemTy)) {
      // Scalar shared → [NumCTA x Elem]
      auto *SlicedTy = ArrayType::get(ElemTy, NumCTAForType);
      NewGV =
          createNoTLSGlobal(SlicedTy, "wrapper_global_" + SM->getName().str());
    } else {
  ...
  for (auto &kv : MapToSlicedGlobal) {
    ...
    for (User *U : Users) {
      ...
      Value *ctaBase = B.CreateInBoundsGEP(New->getValueType(), New,
                                           {ConstantInt::get(I64, 0), cta64});

      // (3) If original use was a GEP, append its indices; else use pointer
      // cast
      ...
      // (4) For other uses (load/store/bitcast/phi, etc.): replace SM with
      // per-CTA base
      //     Cast pointer if type/address space differ
      Value *TypedBase = ctaBase;
      Type *WantPtrTy =
          SM->getType(); // original shared pointer type (including AS)
...
void mem_share2local(llvm::Module *M) {
  ...
  auto isDynamicMarker = [&](const GlobalVariable *GV) {
    if (!isSharedAS(GV))
      return false;
    if (GV->hasInitializer())
      return false; // treat extern only as dynamic marker
  ...
  for (auto &GV : M->globals())
    if (isSharedAS(&GV))
      allShared.push_back(&GV);
  if (allShared.empty())
    return;

  // TLS intrinsic decl
  auto tlsAddr = [&](GlobalVariable *GV) -> FunctionCallee {
    return Intrinsic::getDeclaration(M, Intrinsic::threadlocal_address,
                                     {GV->getType()});
  };
  ...
  for (Function &F : *M) {
    ...
    // For this function, pick only AS3 globals actually used (also break CE
    // uses)
    ...
    for (auto *GV : allShared) {
      ...
      if (!used)
        continue;
      if (isDynamicMarker(GV))
        usedDynamic.push_back(GV);
      else
        usedStatic.push_back(GV);
    }
    if (usedStatic.empty() && usedDynamic.empty())
      continue;
    ...
    // If there is no first barrier: handle immediately in entry (do NOT add
    // group_off)
    if (!FirstBarrier) {
      ...
      for (auto *GV : usedDynamic) {
        ...
        dynAlign = std::max<uint64_t>(
            dynAlign, DL.getABITypeAlign(AT->getElementType()).value());
      }
      ...
      Value *stride = B.CreateAdd(ConstantInt::get(I32, staticAligned),
                                  dynBytes, "total_stride");

      // base_group = vx_local_alloc(stride)  // ★ internally already applies
      // gid*stride
      ...
      for (auto &it : layout) {
        auto *GV = it.GV;
        Value *p = B.CreateGEP(I8, bgrp, ConstantInt::get(I32, it.off),
                               GV->getName() + ".i8");
        Value *t = B.CreatePointerCast(
            p,
      ...
      if (!usedDynamic.empty()) {
        Value *dynBase = B.CreateGEP(
            I8, bgrp, ConstantInt::get(I32, staticAligned), "dyn_base_i8");
        for (auto *GV : usedDynamic) {
          Value *t = B.CreatePointerCast(dynBase, GV->getType(),
                                         GV->getName() + ".dynptr");
          repl[GV] = t;
        }
      }
      ...
      for (auto *GV : usedStatic)
        replaceUsesInFunc(GV, repl[GV]);
      for (auto *GV : usedDynamic)
        replaceUsesInFunc(GV, repl[GV]);
      continue;
    }

    // --- 1) Split at the first barrier: [Pre] -> [WithBarrier (barrier is
    // first inst)]
    BasicBlock *WithBarrierBB =
        EntryBB->splitBasicBlock(FirstBarrier, "with.barrier");
    ...
    auto mkIsZero = [&](GlobalVariable *GV, const char *nm) -> Value * {
      if (!GV)
        return ConstantInt::get(I1,
                                1); // if that axis does not exist, assume zero
    ...
    // --- 2) AllocBB: compute static/dynamic sizes → single allocation → store
    // to global
    struct Item {
      GlobalVariable *GV;
      uint32_t off;
    ...
    Value *stride = IB_Alloc.CreateAdd(ConstantInt::get(I32, staticAligned),
                                       dynBytes, "total_stride");
    ...
    Instruction *AfterBarrierInst =
        WithBarrierBB->getFirstNonPHI()->getNextNode();
    ...
    // load base
    Value *bgrp_all = IB_After.CreateLoad(SharedBaseGV->getValueType(),
                                          SharedBaseGV, "base_group");
    ...
    for (auto &it : layout) {
      auto *GV = it.GV;
      Value *pi8 = IB_After.CreateGEP(
          I8, bgrp_all, ConstantInt::get(I32, it.off), GV->getName() + ".i8");
    ...
    // create replacement pointer(s) for dynamic region (base + staticAligned →
    // [0 x T]*)
    if (!usedDynamic.empty()) {
      Value *dynBase = IB_After.CreateGEP(
          I8, bgrp_all, ConstantInt::get(I32, staticAligned), "dyn_base_i8");
      for (auto *GV : usedDynamic) {
        Value *tp = IB_After.CreatePointerCast(dynBase, GV->getType(),
                                               GV->getName() + ".dynptr");
        repl[GV] = tp;
      }
    }
        ...
        if (AS == 4) { // find a constant memory
          ...
          if (auto array_type = dyn_cast<ArrayType>(element_type)) {
            if (constant_memory->hasExternalLinkage() &&
              ...
              std::cout << "(debug) const mem case 2"
                        << constant_memory->hasExternalLinkage() << " "
                        << array_type->getArrayNumElements() << std::endl;
              ...
              llvm::GlobalVariable *global_memory = new llvm::GlobalVariable(
                  *M, array_type, false, llvm::GlobalValue::ExternalLinkage,
                  constant_memory->getInitializer(), new_name, NULL,
            ...
            // TODO:  need to implement structure type definition for kernel
            // wrapper
