...
bool isCGThreadBlockSync(const std::string &FuncNameStr) {
  // cg::
  bool hasCgnamespace =
      FuncNameStr.find("cooperative_groups") != FuncNameStr.npos;
  // synchronize(thread_block) or sync(thread_block)
  bool hasBlockSynchronize =
      (FuncNameStr.find("synchronize") != FuncNameStr.npos ||
       FuncNameStr.find("sync") != FuncNameStr.npos) &&
      FuncNameStr.find("thread_block") != FuncNameStr.npos;
  // thread_block::sync()
  bool hasBlockSync = FuncNameStr.find("sync") != FuncNameStr.npos &&
                      FuncNameStr.find("thread_block") != FuncNameStr.npos;
...
bool isCGThreadGroupSync(const std::string &FuncNameStr) {
  ...
  bool hasBlockSynchronize =
      (FuncNameStr.find("sync") != FuncNameStr.npos ||
       FuncNameStr.find("synchronize") != FuncNameStr.npos) &&
      FuncNameStr.find("thread_group") != FuncNameStr.npos;
  // thread_block::sync()
  bool hasBlockSync = FuncNameStr.find("sync") != FuncNameStr.npos &&
                      FuncNameStr.find("thread_group") != FuncNameStr.npos;
...
// Check if the given call instruction is a cooperative group symchronization
// related calls
...
bool isWarpSync(const std::string &FuncNameStr) {
  return FuncNameStr.find("syncwarp") != FuncNameStr.npos ||
         FuncNameStr == "llvm.nvvm.bar.warp.sync";
}
