  ...
  for (Module::iterator i = M->begin(), e = M->end(); i != e; ++i) {
    ...
    if (share_memory != NULL) {
      ...
      llvm::FunctionType *LaunchFun2 = FunctionType::get(
          PointerType::get(PointerType::get(Int32T, 0), 0), false);
    ...
    for (Function::const_arg_iterator ii = F->arg_begin(), ee = F->arg_end();
      ...
      // calculate addr
      // LLVM 18 changes
      // Value *GEP = createGEP(Builder, input_arg, ConstantInt::get(Int32T,
      // idx));
    ...
    for (auto BB = F->begin(); BB != F->end(); ++BB) {
      for (auto BI = BB->begin(); BI != BB->end(); BI++) {
        if (auto Call = dyn_cast<CallInst>(BI)) {
          ...
          if (isWarpSync(func_name) || func_name == "llvm.nvvm.barrier0" ||
...
void create_kernel_wrapper_function(llvm::Module *M) {
  ...
  while(readfile >> kernel_idx_tmp)
  {
    ...
    std::cout << "looking for kernel name " << kernel_name_tmp << " with " <<
  kernel_idx_tmp << std::endl;
  ...
  */
    ...
    for (auto BB = F->begin(); BB != F->end(); ++BB) {
      for (auto BI = BB->begin(); BI != BB->end(); BI++) {
        if (auto Call = dyn_cast<CallInst>(BI)) {
          ...
          std::cout << "currently looking function is " << func_name
                    << std::endl;

          // Find the kernel function name
          if (isKernelFunction(M, Call->getCalledFunction()) &&
              std::find(wrapper_name.begin(), wrapper_name.end(),
                        func_name + "_wrapper") == wrapper_name.end()) {
            std::cout
                << "Found the kernel name for the kernel_wrapper.cpp, it is "
                << func_name << "with number of arg " << func_arg_size
                << " kernel_idx: " << std::to_string(kernel_idx) << std::endl;
            // remove _Z24
  ...
  ss << "#include <stdint.h>\n"
     ...
     << "\n"
        ...
        // "int (* volatile vx_shfl_up_dummy)(size_t, int, int, int) =
        // vx_shfl_up;\n" "int (* volatile vx_shfl_down_dummy)(size_t, int, int,
        // int) = vx_shfl_down;\n" "int (* volatile vx_shfl_bfly_dummy)(size_t,
        // int, int, int) = vx_shfl_bfly;\n" "int (* volatile
        // vx_shfl_idx_dummy)(size_t, int, int, int) = vx_shfl_idx;\n"
        "\n"
        "\n"
  ...
  for (auto f : wrapper_name) {
    ss << "void cuda_" << f
       << "(void* args) {\n"
          ...
          "\n"
          "//    vx_printf(\"kernel_warpper: group=(%d, %d) thread=(%d, "
  ...
  ss << "};\n"
        ...
        "(kernel_arg_t*)csr_read(VX_CSR_MSCRATCH); \n"
        ...
        "    vx_printf(\"CHECK: cudamemcpytosymbol, number of "
        "cudamemcpytosymbol=%d \\n\", memcpy_symbol_array[0]);\n"
        "    int memcpy_symbol_idx = 0;\n"
        "    while (memcpy_symbol_idx < memcpy_symbol_array[0]) {\n"
        "        auto dst_addr = "
        "(uint64_t*)memcpy_symbol_array[memcpy_symbol_idx * 3 + 1];\n"
        "        auto src_addr = "
        "(uint64_t*)memcpy_symbol_array[memcpy_symbol_idx * 3 + 2];\n"
        "        auto size = (size_t)memcpy_symbol_array[memcpy_symbol_idx * 3 "
        "+ 3];\n"
        "        memcpy(dst_addr, src_addr, size);\n"
        "        //vx_printf(\"memcpy_symbol[%d]: dst_addr=0x%p, "
        "src_addr=0x%p, size=%lu\\n\", memcpy_symbol_idx, dst_addr, src_addr, "
        "size);\n"
        "        memcpy_symbol_idx++;}}\n"

        //      "    auto additional_info =
        //      (uint64_t*)KERNEL_ARG_ADDITIONAL_INFO_BASE_ADDR; \n"
        // //"    vx_printf(\"additional_info[0]: %lu\\n\",
        // additional_info[0]);\n" "    if (additional_info[0] != 0) {\n" "
        // vx_printf(\"CHECK: cudamemcpytosymbol\");\n" "       int
        // additional_info_idx = 0;\n" "       while (additional_info_idx <
        // additional_info[0]) {\n" "           auto dst_addr =
        // (uint64_t*)additional_info[additional_info_idx * 3 + 1];\n" " auto
        // src_addr = (uint64_t*)additional_info[additional_info_idx * 3 +
        // 2];\n" "           auto size =
        // (size_t)additional_info[additional_info_idx * 3 + 3];\n" "
        // memcpy(dst_addr, src_addr, size);\n" " additional_info_idx++;}}\n"

        "    //vx_printf(\"sizeof everything %d %d %d\\n\", "
        "sizeof(*kernel_arg), sizeof(*ctx), sizeof(ctx->printf_buffer)); \n"
        "    //vx_printf(\"base: 0x%lx\\n\", KERNEL_ARG_BASE_ADDR); \n"
        "    //vx_printf(\"kernel#%d (callback:0x%lx): gridDim=(%d, %d, %d), "
        "blockDim=(%d, %d, %d), args=(0x%llx, 0x%llx, 0x%llx, 0x%llx, 0x%llx, "
        "0x%llx)\\n\", \n"
        "    //    kernel_arg->kernel_idx, callbacks[kernel_arg->kernel_idx], "
        "ctx->num_groups[0], ctx->num_groups[1], ctx->num_groups[2], \n"
        "    //    ctx->local_size[0], ctx->local_size[1], "
        "ctx->local_size[2],\n"
  ...
  /*if(mapping_type == 1)
    ss << "    vx_spawn_kernel_cm(ctx, callbacks[kernel_arg->kernel_idx],
  args);\n"; else ss << "    vx_spawn_kernel(ctx,
  callbacks[kernel_arg->kernel_idx], args);\n";
  */

  // ss << "    return vx_spawn_threads(1, ctx->num_groups, ctx->local_size,
  // (vx_kernel_func_cb)callbacks[0], kernel_arg); \n";
  int schedule = 0;
  ...
  if (schedule == 0) { // thread mapping
    ss << "    return vx_spawn_threads(3, ctx->num_groups, nullptr, "
          "(vx_kernel_func_cb)callbacks[kernel_arg->kernel_idx], args); \n";
  } else if (schedule == 1) { // block mapping
    std::cerr << "Block mapping is not implemented yet\n";
  } else { // one to one mapping
    ss << "    return vx_spawn_threads(3, ctx->num_groups, ctx->local_size, "
          "(vx_kernel_func_cb)callbacks[kernel_arg->kernel_idx], args); \n";
  }
  ss << "\n"

        // --------------- CUDAMemcpytosymbol -----------4temp disabled
        // "    // Copy back the additional info (changed by the kernel,
        // Cudamemcpytosymbol)\n" "if (memcpy_symbol_array[0] != 0) {\n" "
        // vx_printf(\"CHECK: cudamemcpytosymbol, number of
        // cudamemcpytosymbol=%d \\n\", memcpy_symbol_array[0]);\n" "    int
        // memcpy_symbol_idx = 0;\n" "    while (memcpy_symbol_idx <
        // memcpy_symbol_array[0]) {\n" "        auto src_addr =
        // (uint64_t*)memcpy_symbol_array[memcpy_symbol_idx * 3 + 1];\n" " auto
        // dst_addr = (uint64_t*)memcpy_symbol_array[memcpy_symbol_idx * 3 +
        // 2];\n" "        auto size =
        // (size_t)memcpy_symbol_array[memcpy_symbol_idx * 3 + 3];\n" "
        // memcpy(dst_addr, src_addr, size);\n" " vx_printf(\"memcpy_symbol[%d]:
        // dst_addr=0x%p, src_addr=0x%p, size=%lu\\n\", memcpy_symbol_idx,
        ...
        /*"    // Copy back the additional info (changed by the kernel,
           ...
           additional_info[0]) {\n" "           auto src_addr =
           (uint64_t*)additional_info[additional_info_idx * 3 + 1];\n" " auto
           dst_addr = (uint64_t*)additional_info[additional_info_idx * 3 +
           2];\n" "           auto size =
           (size_t)additional_info[additional_info_idx * 3 + 3];\n" "
           memcpy(dst_addr, src_addr, size);\n" " additional_info_idx++;}}\n"
